...
// ----------------------------------------------------------------------
// FUNCIÓN NUEVA: Selecciona múltiples líneas por una lista de IDs (WHERE IN)
// ----------------------------------------------------------------------
//
// Esta función utiliza la sintaxis segura de GORM para la cláusula WHERE IN.
// La función espera un slice de IDs ([]int) y devuelve un slice de objetos Linea.
func SelectLinesByIDs(lineIDs []int) []models.Linea {
	var lineas []models.Linea

	// Si el slice de IDs está vacío, devuelve un slice vacío
	if len(lineIDs) == 0 {
		return lineas
	}

	// GORM automáticamente convierte el slice 'lineIDs' a la cláusula SQL 'IN (?, ?, ?)'
	// y maneja la seguridad de los parámetros.
	result := con.DB.Preload("Descripcion_linea").Where("id IN ?", lineIDs).Find(&lineas)

	if result.Error != nil {
		log.Printf("Error al buscar líneas por IDs %v: %v", lineIDs, result.Error)
		// Dependiendo de tu lógica de errores, podrías devolver el error aquí.
		// Por ahora, solo logeamos y devolvemos lo que se haya encontrado (o nada).
	}
